...
export const LongCaption = (inputArgs, context) => {
  const args = {
    ...inputArgs,
    caption:
      "This is a very long caption that will demonstrate the truncation behavior. It contains **highlighted** terms and continues with more text to ensure it exceeds the available space and triggers the ellipsis. The quick brown fox jumps over the lazy dog. Lorem ipsum dolor sit amet, consectetur adipiscing elit.",
...
export const InModal = (inputArgs, context) => {
  const args = {
    ...
    inModal: true,
    caption:
      "This is a long caption that would normally be truncated, but since it's in modal view, the entire text should be visible. It contains **highlighted** terms and continues with more text.",
    theme: themeFromContext(context),
  };
  return <ReportCaption {...args} />;
};
...
export const Embedded = (inputArgs, context) => {
  const args = {
    ...
    embedded: true,
    caption:
      "This caption is displayed in embedded mode which affects its scaling behavior. It contains **highlighted** text.",
    theme: themeFromContext(context),
  };
  return <ReportCaption {...args} />;
};
...
export const WithPadding = (inputArgs, context) => {
  const args = {
    ...
    padding: 20,
    caption:
      "This caption has extra padding applied. It contains **highlighted** text.",
    theme: themeFromContext(context),
  };
  return <ReportCaption {...args} />;
};
export const WithPhylopics = (inputArgs, context) => {
  ...
  combinedStore.dispatch({
    ...
    payload: {
      ...
      fileUrl:
        "https://static.vecteezy.com/system/resources/previews/015/152/952/original/url-icon-design-for-web-interfaces-and-applications-png.png",
      dataUri:
...
export const MultipleHighlights = (inputArgs, context) => {
  const args = {
    ...inputArgs,
    caption:
      "This caption contains **multiple** highlighted terms at **different** points in the text to test **formatting**.",
    theme: themeFromContext(context),
  };
  return <ReportCaption {...args} />;
};
